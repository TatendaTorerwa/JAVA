...
//Student Managemnet class to handle student records
class StudentManagement {
    ...
    public static void updateStudent(String name, int ID, int age, float grade) {
        for (Student t : studentList) {
            if (t.ID == ID) {
                t.name = newName;
                t.age = newAge;
...
public class RecordManagementSystem {
    public static void main(String[] args) {
        ...
        switch (choice) {
            case 1: //add student
                ...
                float grade = input.getFloatInput(input);
            ...
            case 2: //update student
                ...
                int newName = input.nextLine();

                System.out.println("Enter New Age: ");
                int updateID = getIntInput(input);

                System.out.println("Enter New Grade: ");
                int updateID = getFloatInput(input);
    ...
    private static int getIntTnput(Scanner input) {
        while (!input.hasNextInt()) {
            System.out.println("Invalid input. Plaese enter a valid number.");
            input.next(); //clear invalid input
        }
